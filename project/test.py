...
class App(tk.Tk):
    ...
    def volatility_analysis(self, ticker, stock_data, risk_tolerance):
        ...
        if model.coef_ < 1.2 and model.coef_ > 0.8 and risk_tolerance == "medium":
            ...
            return False
    def main_analysis(self, ticker_list, portfolio_diversification, investment_amount, risk_tolerance):  # Add risk_tolerance parameter
        analysis_results = {}
        ...
        # Randomize data (better to compare all analyst ratings and sort by that)
        random.shuffle(ticker_list)

        for ticker in ticker_list:
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                stock_data = self.fetch_stock_data(ticker)  # Use self.fetch_stock_data
            cleaned_data = self.clean_stock_data(stock_data)
            recommendation_result = self.recommendation_analysis(ticker)
            volatility_result = self.volatility_analysis(ticker, stock_data, risk_tolerance)  # Use self.volatility_analysis

            if recommendation_result and volatility_result:  # Ensure volatility_result is True
                analysis_results[ticker] = {
                    'Recommendation Analysis': recommendation_result,
                    'Volatility Result': volatility_result
                }
